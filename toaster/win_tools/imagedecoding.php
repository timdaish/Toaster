		...
		switch($seg_hdr)
		{
			...
			case 'ffe1':
				...
				else
				{
					...
					$res = array();
                    if($OS == "Windows")
					    exec('perl "\win_tools\ExifToolPerl\exiftool.pl" -xmp -b ' . escapeshellarg($lf),$res);
                    else
...
function decodeGIF($content,$lf)
{
    ...
    $res = array();
    if($OS == "Windows")
        exec('perl "\win_tools\ExifToolPerl\exiftool.pl" -j '. escapeshellarg($lf),$rawgifdata);
    else
    ...
    $res = array();
    if($OS == "Windows")
        exec('perl "\win_tools\ExifToolPerl\exiftool.pl" -verbose '. escapeshellarg($lf),$rawgifinfo);
    else
...
function decodePNG($content,$lf)
{
  ...
  //pngcrush -n -v <img> to identify text chunnks
    ...
    $windows_cmd = "win_tools\exiftool" .' -v ' .escapeshellarg($lf);
...
//  echo('</pre>');
  ...
  $res = array();
  if($OS == "Windows")
    exec('perl "\win_tools\ExifToolPerl\exiftool.pl" -j '. escapeshellarg($lf),$rawpngdata);
  else
...
// Get the content that is in the buffer and put it in your file //
  ...
  if(isset($pngdata->XMPToolkit))
    ...
    if($OS == "Windows")
        exec('perl "\win_tools\ExifToolPerl\exiftool.pl" -v '. escapeshellarg($lf),$pngstructure);
    else
