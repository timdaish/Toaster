...
function setImage(side, pathBase, codec, setText) {
    ...
    var path = ''
    // use original path for original selection
    ...
    {
console.log(pathBase, '/', urlFile, '.', codec);
if(urlFile != "default")
      path = urlFolder.concat(pathBase, '/', urlFile, '.', codec);
else
    return false;
    }
    ...
    xhr.onload = function() {
        setText( pathBase + ": " + (xhr.response.byteLength/1024).toFixed(1) + " KB" );
        ...
        if (codec == 'bpg') {
            ...
            image.onerror = function() {
                ...
                } else { console.error("No support for " + urlFile); }
            };
            image.src = blobPath;
        }
    };
...
function setSide(side) {
    ...
    //console.log("selected folder: " + urlFolder);
    //console.log("selected pathbase: " + pathBase);

    if (pathBase == 'jpg' || pathBase == 'png'  ) {
        ...
        if(pathBase == 'jpg')
        {
            if(isRight)
            {
                var selID = rightQual.options[rightQual.selectedIndex].value; // get id
                var selchar1 = selID.substr(0,1);
                //console.log('jr: ' + selID);
                if(selchar1 != 'j')
                    rightQual.options[4].selected = true;
            }
            else
            {
                var selID = leftQual.options[leftQual.selectedIndex].value; // get id
                var selchar1 = selID.substr(0,1);
                //console.log('jl: ' + selID);
                if(selchar1 != 'j')
                leftQual.options[4].selected = true;
            }
        }
         if(pathBase == 'png')
        {
            if(isRight)
            {
                var selID = rightQual.options[rightQual.selectedIndex].value; // get id
                var selchar1 = selID.substr(0,1);
                //console.log('pr: ' + selID);
                if(selchar1 != 'p')
                    rightQual.options[12].selected = true;
            }
            else
            {
                var selID = leftQual.options[leftQual.selectedIndex].value; // get id
                var selchar1 = selID.substr(0,1);
                //console.log('pl: ' + selID);
                if(selchar1 != 'p')
                leftQual.options[12].selected = true;
            }

        }


        //console.log('final selected id: ' + selID);
        var quality = whichQual.options[whichQual.selectedIndex].innerHTML.toLowerCase();
        pathBase =  quality;
    } else {

        var quality = '';
        // if not png or jpg, select quality blank
        if(isRight)
            rightQual.options[18].selected = true;
        else
            leftQual.options[18].selected = true;
        whichQual.disabled=true;
        pathBase =  pathBase + quality;
    }
...
function getWindowsOptions(tf,oi,fn) {

//   console.log('si.js: toasterurlfolder =' + tf );
//   console.log('si.js: original image = ' + oi);
//   console.log('si.js: img filename = ' + fn);
  toasturlfolder = tf;
  originalimage = oi;
  imgfn = fn;

    var lc = toasturlfolder.slice(-1);
    if (lc == "/")
        urlFolder =  toasturlfolder + "_Optimised_Images/";
    else
        urlFolder =  toasturlfolder + "/_Optimised_Images/";
    if (window.location.hash) {
        ...
        if (hashArr[1]) {
            if (hashArr[1] == "psycomp") {
                urlFolder = toasturlfolder + "/psycomp/";
