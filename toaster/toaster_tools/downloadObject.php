...
{
	global $url,$filepath_domainsavedir,$filepath_domainsaverootdir,$filepath_basesavedir,$ua,$host_domain,$arrayOfObjects,$bool_b64,$sourceurlparts,$body,$objcount,$objcountimg, $arrayListOfImages,$arrayPageObjects,$debug,$boolGzipRoot,$compressionlevel,$totfilesize,$embeddedfile_count,$embeddedcount,$objcountcss,$objcountscript,$objcountimg,$totbytesdownloaded,$noof404Errors,$pagespeedcount,$objcountfont,$plainsvgid,$plainsvgelement,$OS;
...
//echo("<br/><br/>Downloading object: key: $key  objno: $pageobjectno;  file: ".$value.";<br/>local filename: $local<br/>");
...
//echo($filepathnameofsaveobject." embedded font name = ".$fontname.'<br/>');
			...
			else
			{
                    ...
                    if($thisstatuscode != '')
                    {
                      ...
                      // echo ("object " . $key . ": saved as " . $filepathnameofsaveobject . "<br/>");
                    ...
                    //{
    					...
    					if(strtolower(substr($sourcefile,0,4)) != "http")
    					{
    						...
    						{
    							debug("prefixing http:",$sourcefile);
    							$sourcefile = "http:" . $sourcefile;
    						}
    						else
    						{
    ...
    //echo("SAVE FILE PATHNAME: ". $filepathnameofsaveobject . "<br/>");
    					...
    					$sourcefileNoSpaces = str_replace(" ","%20",$sourcefile);
    					...
    					//echo ("protocol and age: ".$protocol . " " .$age."<br/>");
    ...
    //echo ("response code: " .$responsecode. "; sc: " . $sc . "<br/>");
...
//echo __FUNCTION__ . " line " . __LINE__ . 'HTTP Success retrieving file: ' .$sourcefile.";<br/> CURL Redir count: " . $redir_count . "; FINAL HTTP Status: ". $sc."; Hdr size: ". $hdrlength."; Content length transmitted: ".$contentlength."<br/>";
						...
						if ($redir_count == 0 or isset($redir_count) == false )
						{
							...
							//echo("Update dl page object array: ".$key . "; noof redirs = " . $redir_count."<br/>");
							// update page object array
							$arr = array(
							"Object type" => $objtype,
							...
							UpdateDomainLocationFromHeader($rawurl,$xservedby,$xpx,$xedgelocation,$server,$cfray,$xcdngeo,$xcdn,"downloadobject: ".$rawurl);
		                    $finalurl = $rawurl;
						}
						else
						{
						...
						if(intval($sc) == 200)
						{
                           ...
                           case "font/woff2":
							...
							if ($boolTextType == true and $redir_count == 0)
							{
								...
								{
									debug("This file is already gzipped when served","");
								}
								...
								//echo("Checking minification: $lfn<br/>");
...
//echo("Analysing Image: ". $lfn."; size: ".$fsize."</br>");
						...
						{
							//echo ("image file was not saved properly<br/>");
							$truemimetype = $mimetype;
							addErrors($sourcefile,"not downloaded successfully as ". $lfn);
						}
					}
...
//echo("GIF structure: " . $structure ."<br/>");
                            ...
                            if($xmpbytes > 0)
    						{
    						   //$res = array();
    							...
    							$res = array();
                                if($OS == "Windows")
    							    exec('perl "\toaster_tools\ExifToolPerl\exiftool.pl" -xmp -b ' . $lfn,$res);
                                else
...
//echo ("<br/>Decoding JPEG: $sourcefile<br/>");
						...
						//echo("updating page object - JPG<br/>");
						addUpdatePageObject($arr);
						...
						if($iccbytes > 0)
						{
							$res = array();
                            if($OS == "Windows")
							    exec('perl "\toaster_tools\ExifToolPerl\exiftool.pl" -icc_profile -b ' . $lfn,$res);
                            else
						...
						if($exifbytes > 0)
						{
							...
							// EXIFTOOL - exif
							//$res = array();
							//exec('perl "\toaster_tools\ExifToolPerl\exiftool.pl" -E ' . $filepathnameofsaveobject,$res);
							//$exifinfo = $res;
